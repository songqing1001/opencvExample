 ...
 // from the fundamental matrix
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            Mat imgTmp;
            cvtColor(img, imgTmp, COLOR_GRAY2BGR);
            remap(img, rimg, rmap[k][0], rmap[k][1], INTER_LINEAR);
            cvtColor(rimg, cimg, COLOR_GRAY2BGR);

          //  imshow("k", imgTmp);
            cout<<"good image :"<< goodImageList[i*2+k]<<endl;
            imshow("k", cimg);

            char sFileName[256] = {0};
            snprintf(sFileName, 255, "%s.sterocalc.jpg", goodImageList[i*2+k].c_str());
            imwrite(sFileName, cimg);
    ...
    ////////////////////for my only test
    {
        Mat img = imread("/data_1/songqing/opencv/example/chapter18/1calc.jpg", 0), rimg, cimg;
    ...
    {
        Mat img = imread("/data_1/songqing/opencv/example/chapter18/2calc.jpg", 0), rimg, cimg;
        remap(img, rimg, rmap[1][0], rmap[1][1], INTER_LINEAR);
        cvtColor(rimg, cimg, COLOR_GRAY2BGR);
...
int main(int argc, char** argv)
{
    ...
    imagelistfn = "/data_1/songqing/opencv/example/image/stereoChessboard/path.yml";
